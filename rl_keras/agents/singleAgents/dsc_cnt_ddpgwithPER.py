...
class DDPGAgent(Agent):
    ...
    def critic_loss(self, y_true, y_pred, importance_weights):
        ...
        return K.mean(loss * importance_weights, axis=-1)
    ...
    def critic_optimizer(self):
        ...
        imp_wgt = K.placeholder(shape=(None, 1))
        # actions = K.placeholder(shape=(None, self.nb_actions))
        Qvalue = self.critic.output

        loss = self.critic_loss(y_true=discounted_reward, y_pred=Qvalue, importance_weights=imp_wgt)
        loss = K.mean(loss)
        ...
        train_fn = K.function(inputs=self.critic.input + [discounted_reward, imp_wgt, K.learning_phase()],
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                if not self.prioritized:
                    ...
                    # Make importance_weights the same shape as the other tensors that are passed into the trainable env
                ...
                critic_input = state0_batch_with_action + [targets] + [importance_weights] + [self.training]
                ...
                if self.prioritized:
                    ...
                    y_true = targets
                    y_pred = self.critic.predict_on_batch(state0_batch_with_action)
