...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        if hasattr(model.output, '__len__') and len(model.output) > 1:
            raise ValueError('Model "{}" has more than one output. DQN expects a env that has a single output.'.format(model))
        if model.output._keras_shape != (None, self.nb_actions):
            raise ValueError('Model output "{}" has invalid shape. DQN expects a env that has one dimension for each action, in this case {}.'.format(model.output, self.nb_actions))
        ...
        if self.enable_dueling_network:
            # get the second last layer of the env, abandon the last layer
    ...
    def get_config(self):
        ...
        config['env'] = get_object_config(self.model)
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        # We never train the target env, hence we can set the optimizer and loss arbitrarily.
        ...
        # Create trainable env. The problem is that we need to mask the output since we only
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            if self.enable_double_dqn:
                ...
                # highest Q value wrt to the online env (as computed above).
                target_q_values = self.target_model.predict_on_batch(state1_batch)
                assert target_q_values.shape == (self.batch_size, self.nb_actions)
                q_batch = target_q_values[range(self.batch_size), actions]
            else:
                # Compute the q_values given state1, and extract the maximum for each sample in the batch.
                # We perform this prediction on the target_model instead of the env for reasons
...
class NAFAgent(AbstractDQNAgent):
    ...
    def load_weights(self, filepath):
        self.combined_model.load_weights(filepath)  # updates V, L and mu env since the weights are shared
        self.update_target_model_hard()
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        # Create target V env. We don't need targets for mu or L.
