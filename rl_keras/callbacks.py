...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        template = '{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode}, duration: {duration:.3f}s, episode steps: {episode_steps}, steps per second: {sps:.0f}, episode reward: {episode_reward:.3f}, mean reward: {reward_mean:.3f} [{reward_min:.3f}, {reward_max:.3f}], {metrics}'
        variables = {
            'step': self.step,
...
class FileLogger(Callback):
    ...
    def on_train_begin(self, logs):
        """ Initialize env metrics before training """
        self.metrics_names = self.model.metrics_names

    def on_train_end(self, logs):
        """ Save env at the end of training """
        self.save_data()
...
# TODO : is it possible to create every episodic callback and every callback which iteration  interval size is step size in same callback class? Interval variable can not be shared by two kinds callbacks.
class ModelIntervalCheckpoint(Callback):
    ...
    def on_step_end(self, step, logs={}):
        ...
        if self.verbose > 0:
            print('Step {}: saving env to {}'.format(self.total_steps, filepath))
        self.model.save_weights(filepath, overwrite=True)


class TrainHistoryLogCallback(Callback):
    ...
    def __init__(self, file_path=None, plot_interval=10000, data_for_plot=['episode_reward', 'nb_episode_steps']):
        ...
        """
        super(TrainHistoryLogCallback, self).__init__()
    ...
    def on_episode_end(self, episode, logs):
        # cumulate data into list for plot in the end
