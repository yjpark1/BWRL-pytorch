...
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        try:
            while self.step < nb_steps:
                # hold training until service_flag = 1
                ...
                if observation is None:  # start of a new episode
                    ...
                    step_rnd = 0
                    while step_rnd < nb_random_start_steps:
                        # hold training until service_flag = 1
                        if gvar.service_flag == 0:
                            time.sleep(1e-5)
                            continue
                        ...
                        if done:
                            ...
                            break
                        step_rnd += 1
                    logger.info('random action: ' + str(step_rnd))
                    # random play done!
                ...
                step_repeat = 0
                while step_repeat < action_repetition:
                    # hold training until service_flag = 1
                    if gvar.service_flag == 0:
                        time.sleep(1e-5)
                        continue
                ...
                episode_step += 1
                self.step += 1
                # print("self.step: ", self.step, " episode_step: ", episode_step)
    ...
    @property
    def layers(self):
        """Returns all layers of the underlying env(s).

        If the concrete implementation uses multiple internal models,
        this method returns them in a concatenated list.

        # Returns
            A list of the env's layers
        """
        raise NotImplementedError()
...
class Space(object):
    """Abstract env for a space that is used for the state and action spaces. This class has the
